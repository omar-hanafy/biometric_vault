#include "include/biometric_vault/biometric_vault_plugin.h"
...
const char kNamePrefix[] = "io.github.omarhanafy.authpass";
...
#define BIOMETRIC_STORAGE_PLUGIN(obj) \
  (G_TYPE_CHECK_INSTANCE_CAST((obj), biometric_vault_plugin_get_type(), \
                              BiometricVaultPlugin))

#define IS_METHOD(name, equals) \
  strcmp(method, equals) == 0

struct _BiometricVaultPlugin {
  GObject parent_instance;
};

G_DEFINE_TYPE(BiometricVaultPlugin, biometric_vault_plugin, g_object_get_type())
...
biometric_get_schema (void)
{
    static const SecretSchema the_schema = {
        "io.github.omarhanafy.BiometricVault", SECRET_SCHEMA_NONE,
        {
...
static void
biometric_vault_plugin_handle_method_call(BiometricVaultPlugin *self,
...
static void biometric_vault_plugin_dispose(GObject* object) {
  G_OBJECT_CLASS(biometric_vault_plugin_parent_class)->dispose(object);
}

static void biometric_vault_plugin_class_init(BiometricVaultPluginClass* klass) {
  G_OBJECT_CLASS(klass)->dispose = biometric_vault_plugin_dispose;
}

static void biometric_vault_plugin_init(BiometricVaultPlugin* self) {}

static void method_call_cb(FlMethodChannel* channel, FlMethodCall* method_call,
                           gpointer user_data) {
  BiometricVaultPlugin* plugin = BIOMETRIC_STORAGE_PLUGIN(user_data);
  biometric_vault_plugin_handle_method_call(plugin, method_call);
}

void biometric_vault_plugin_register_with_registrar(FlPluginRegistrar* registrar) {
  BiometricVaultPlugin* plugin = BIOMETRIC_STORAGE_PLUGIN(
      g_object_new(biometric_vault_plugin_get_type(), nullptr));
